...
class LongShort:
  def __init__(self):
    ...
    stockUniverse = ['GOOG', 'DOMO', 'SQ', 'MRO', 'AAPL', 'GM', 'SNAP', 'SHOP', 'SPLK', 'BA', 'AMZN', 'SUI', 'SUN', 'TSLA', 'CGC', 'SPWR', 'CAT', 'MSFT', 'PANW', 'OKTA', 'TWTR', 'TM', 'RTX', 'GS', 'BAC', 'MS', 'TWLO', 'QCOM', 'TTD', 'MAR', 'DIS', 'UAL', 'DAL', 'SQ', 'PYPL', 'RH', 'NVDA', 'ADBE', 'ZM', 'PINS', 'APPN']
    #stockUniverse = ['MAR', 'TSLA', 'DIS', 'UAL', 'SQ', 'PINS', 'QQQ', 'VOO']
  ...
  def run(self):
    ...
    while True:
      ...
      if(self.timeToClose < (60 * 5)):
        ...
        tRebalance.start()
        tRebalance.join()
        print("Rebalance done at "+ str(time.strftime('%X %x %Z')) + ", napping now\n\n")
        markPort(self)
        time.sleep(300)
  ...
  # glee updated function to send limit orders at far touch instead of market orders.
  def submitOrder(self, qty, stock, side, resp):
    far={}
    if(qty > 0):
      try:
          if(side == "buy"):
              far = self.alpaca.get_last_quote(stock).askprice
              self.alpaca.submit_order(stock, qty, side, "limit", "day", far)
              print("Limit order of | " + str(qty) + " " + stock + " " + side + " at " + str(far) + " | completed.")
              resp.append(True)
          else:
              far = self.alpaca.get_last_quote(stock).bidprice
              self.alpaca.submit_order(stock, qty, side, "limit", "day", far)
              print("Limit order of | " + str(qty) + " " + stock + " " + side + " at " + str(far) + " | completed.")
              resp.append(True)
      except:
        print("Order of | " + str(qty) + " " + stock + " " + side + " at " + str(far) + " | did not go through.")
        resp.append(False)
    else:
      print("Quantity is 0, order of | " + str(qty) + " " + stock + " " + side + " at " + str(far) + " | not completed.")
      resp.append(True)
  ...
  def getPercentChanges(self):
    ...
    for i, stock in enumerate(self.allStocks):
      ...
      #print(str(bars[stock[0]][0].o))

  #Function to determine longer trend
  # glee NOT used yet
...
# glee function to mark the current positions value
def markPort(self):
    totalMark=0
    positions = self.alpaca.list_positions()
    for position in positions:
        lastPrice = self.alpaca.get_last_trade(position.symbol).price
